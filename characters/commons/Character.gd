"""
Character will share common properties between all inherited classes.
It initiates a finite state machine depending on the State child nodes.
e.g.
	State (Node2D)
	-- Idle (Node2D)
	-- Move (Node2D)

It will automaticly create the state machine with Idle and Move state.
It also contains all the common physics/behavior state variable shared between all 
...
# signal emitted when a the character change its state.
...
func _initialize_state(initial_state: String = 'idle'):
	...
	for state_node in $States.get_children():
		...
		# inactive damage zone by default since they are controled by the AnimatedSprite	
...
func _change_state(state_name: String) -> void:
	...
	elif state_name in ['hit']: # pushdown automaton
